	...
	public void CompareAndGenerateCells()
	{
		...
		try {
			...
			while ((line1 = br1.readLine()) != null) {
				...
				assert (line2 != null);
				assert (line3 != null);
				String[] col1 = line1.split(",", -1);
				// System.out.println(line1);
				// System.out.println(line2);
				// System.out.println(line3);

				String[] col2 = line2.split(",", -1);
				String[] col3 = line3.split(",", -1);
				assert (col1.length == col2.length);
				assert (col1.length == col3.length);
				numOfTuples++;
	        	...
	        	for (int i = 0; i < fds.length; i++)
	        	{
	        		...
	        		for (FD f: ar)
	        		{
	        			...
	        			for (int lhs_index: f.getLHSColumnIndexes())
	        			{
	  					 ...
	  					 cell1.add(col1[f.getRHSColumnIndex()].trim());
    					 cell2.add(col2[f.getRHSColumnIndex()].trim());
    					 cell3.add(col3[f.getRHSColumnIndex()].trim());
